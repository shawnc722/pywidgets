...
class MediaListWidget(_MediaListFramework):
    ...
    def remove_players(self, players):
        for player in players: self.remove_widget(self.mediawidgets[player])
        self.update()

    def new_players(self, players):
        for player in players:
            self.add_widget(MediaWidget(self, player, self.bus, self.imgsize, self.butsize, self.update_interval),
                            player)
        self.update()
...
class MediaWidget(_MediaFramework):
    def __init__(self, parent: QtWidgets.QWidget, player: str, bus: QDBusConnection, imgsize: int = None, butsize: int = None,
        ...
        self.handler = _MprisHandler(player, bus, update_interval)
        self.handler.add_subscriber(self)
        self.playername = self.handler.data_proxy.call("Get", "org.mpris.MediaPlayer2", "Identity").arguments()[0]
    ...
    def metadatachanged(self):
        self.displaytext = f"{self.handler.title()}<br>{self.handler.artist()}"
...
class _MprisHandler(QtCore.QObject):
    def __init__(self, player: str, bus: QDBusConnection, update_interval: int = 250):
        """
